...
def parse_vcs_url(url):
    ...
    try:
        ...
        if "github.com" in parsed_url.netloc:
            ...
            commit_hash = path_components[4] if len(
                path_components) > 4 and path_components[3] == "commit" else None
            ...
            repository = base_url + "/" + \
                path_components[1] + "/" + path_components[2]
            commit_hash = path_components[4] if len(
                path_components) > 4 and path_components[3] == "commits" else None
        else:
            vcs = None
            repository = None
            commit_hash = None
...
def save_list_cols(df):
    ...
    columns_with_lists = [col for col in df.columns if df[col].apply(
...
def collect_repos(df):
    ...
    df_repo.astype(str).to_sql('repository', utils.conn,
